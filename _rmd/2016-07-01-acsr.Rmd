...
The functions depends on the `acs` and `data.table`, so it is necessary to install those packages before. The `acsr` package is hosted on a github repository and we install it using the `devtools` library :
...
Remember to set the ACS api key, in case you haven't: `r  api.key.install(key="")`. And also rember to check the help documentation and the default values of the functions.
...
The default level is `state` and the state specified is Wisconsin (`state = "WI"`). The `endyear` is 2014, and the confidence level to compute margins of error (MOEs) is 90%.

<h2 class="section-heading">Levels</h2>

The `acsr` function can extract all the level available in the `acs` package. The table below shows the summary level and required levels:
